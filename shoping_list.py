...
while True:
    operation = input("Hey! Choose mode: (1 - control category; 2 - control purchase; 'exit' - exit): ")
    operation = operation.capitalize().replace(' ', '')
    if operation == '1':
        while True:
            print("Your list of category:", category)
            category_menu = input("What do you want to do? (1 - choose category; 2 - add category; 3 - delete category; 'exit' - exit): ")
            category_menu = category_menu.capitalize().replace(' ', '')
            if category_menu == 'Exit':
                break
            elif category_menu == '1':
                while True:
                    name_category = input(f"This is your category {category}. Which do you choose? ('other',.., 'exit' - exit): ")
                    name_category = name_category.capitalize().replace(' ', '')
                    if name_category == 'Exit':
                        ...
                        while True:
                            ...
                            operation_with_purchase = input("What do you want to do with this purchase? (1 - add new purchase; 2 - delete some purchase; 3 - delete all purchase; 'exit' - exit): ")
                            operation_with_purchase = operation_with_purchase.capitalize().replace(' ', '')
                            if operation_with_purchase == 'Exit':
                                ...
                                if len(temporary_purchase) >= 10:
                                    ...
                                    name_new_purchase = name_new_purchase.capitalize().replace(' ', '')
                                ...
                                del_purchase = del_purchase.capitalize().replace(' ', '')
                ...
                name_new_category = input("Input a name for the new category: ")
                name_new_category = name_new_category.capitalize().replace(' ', '')
                all_purchase[name_new_category] = []
                continue
            elif category_menu == '3':
                while True:
                    name_del_category = input("Input a name for the new category: ")
                    name_del_category = name_del_category.capitalize().replace(' ', '')
